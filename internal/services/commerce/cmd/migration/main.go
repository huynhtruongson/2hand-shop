...
func executeMigration(migrationType gomigrate.MigrationType) {
	...
	logger := zerolog.NewZeroLogger(zerolog.Config{Level: cfg.Logger.Level, Environment: cfg.App.Environment, ServiceName: cfg.App.ServiceName})
	...
	migrator, err := gomigrate.NewGoMigrateMigrator(logger, dataSource, migrationDir)
	if err != nil {
